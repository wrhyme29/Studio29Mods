    ...
    public class TrashTheVenueCardController : BirthdayBoyCardController
    {
        ...
        public override IEnumerator Play()
        {
            ...
            List<DestroyCardAction> storedResults = new List<DestroyCardAction>()
                ;
            IEnumerator coroutine = GameController.SelectAndDestroyCards(DecisionMaker, new LinqCardCriteria((Card c) => c.IsEnvironment, "environment"), 2, optional: false, requiredDecisions: 0, storedResultsAction: storedResults, cardSource: GetCardSource());
		...
		private IEnumerator MoveCardsToOwnHand()
		{
			...
			IEnumerator coroutine = GameController.MoveCards(HeroTurnTakerController, criteria, (Card c) => HeroTurnTaker.Hand, numberOfCards: new int?(1),  storedResults: storedResults, cardSource: GetCardSource());
			if (UseUnityCoroutines)
			{
				yield return GameController.StartCoroutine(coroutine);
			}
